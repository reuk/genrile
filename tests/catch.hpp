...
#define INTERNAL_CATCH_REGISTER_TESTCASE(Function, ...) \
    Catch::AutoReg(                                     \
        Function, CATCH_INTERNAL_LINEINFO, Catch::NameAndDesc(__VA_ARGS__));
...
#define INTERNAL_CATCH_REGISTER_TESTCASE(Function, Name, Desc) \
    Catch::AutoReg(                                            \
        Function, CATCH_INTERNAL_LINEINFO, Catch::NameAndDesc(Name, Desc));
#endif
...
public:
    ...
    void optUsage(std::ostream& os,
        ...
        for (it = itBegin; it != itEnd; ++it) {
            ...
            for (std::size_t i = 0;
                 i < (std::max)(usageText.size(), desc.size());
                 ++i) {
...
public:
    IndexTracker(std::string const& name,
                 TrackerContext& ctx,
                 ITracker* parent,
                 int size)
            : TrackerBase(name, ctx, parent)
...
void enforceNoDuplicateTestCases(std::vector<TestCase> const& functions) {
    ...
    for (std::vector<TestCase>::const_iterator it = functions.begin(),
        ...
        if (!prev.second) {
            Catch::cerr() << Colour(Colour::Red) << "error: TEST_CASE( \""
...
void Colour::use(Code _colourCode) {
    static IColourImpl* impl = isDebuggerActive() ? NoColourImpl::instance()
